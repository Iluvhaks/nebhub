...
-- Helper: Wait for local character and HRP to load
...
FTAPTab:CreateToggle({Name="Spawn Kill All (Improved)", CurrentValue=spawnKillAll, Callback=function(value)
    spawnKillAll = value
    if spawnKillAll then
        spawn(function()
            local aboveOffset = Vector3.new(0, 50, 0)
            while spawnKillAll do
                local localChar = LocalPlayer.Character
                local localHRP = localChar and localChar:FindFirstChild("HumanoidRootPart")
                if not localHRP then task.wait(1); continue end

                -- Teleport local player above each player in line, grab and send them far away
                for _, p in pairs(Players:GetPlayers()) do
                    if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                        local targetHRP = p.Character.HumanoidRootPart

                        -- Teleport above player
                        localHRP.CFrame = targetHRP.CFrame + aboveOffset
                        task.wait(0.3)

                        -- Attempt to grab with line
                        local mouse = LocalPlayer:GetMouse()
                        local mousePos = mouse.Hit.Position
                        local line = Drawing.new("Line")
                        line.Visible = true
                        line.From = Camera.ViewportSize / 2
                        line.To = Camera:WorldToViewportPoint(targetHRP.Position)
                        line.Color = Color3.new(1, 0, 0)
                        task.wait(0.2)

                        -- On release, send player far away
                        if flingEnabled then
                            local bv = Instance.new("BodyVelocity")
                            bv.MaxForce = Vector3.new(1e9, 1e9, 1e9)
                            bv.Velocity = (targetHRP.CFrame.LookVector) * 50000
                            bv.Parent = targetHRP
...
-- TSB TAB
TSBTab:CreateToggle({Name="TSB Autofarm", CurrentValue=false, Callback=function(v)
    autofarmEnabled = v
    if v then
        spawn(function()
                    ...
                    if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild(TargetPart) then
                        ...
                        if hum and hum.Health > 0 then
                            if TeamCheck and p.Team == LocalPlayer.Team then
                                ...
                                -- Tween to target's position continuously while alive
                                local tweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Linear)
                                local tgtPos = p.Character.HumanoidRootPart.Position + Vector3.new(0,3,0)
                                local tween = TweenService:Create(localHRP, tweenInfo, {CFrame = CFrame.new(tgtPos)})
                                tween:Play()

                                while hum.Health > 0 and autofarmEnabled do
                                    task.wait(0.1)
                                    -- Keep tweening closer in case they move
                                    local newPos = p.Character.HumanoidRootPart.Position + Vector3.new(0,3,0)
                                    tween:Cancel()
                                    tween = TweenService:Create(localHRP, tweenInfo, {CFrame = CFrame.new(newPos)})
                                    tween:Play()

                                    -- Virtual input moves (1,2,3,4 keys)
                                    if UserInput.TouchEnabled then
                                        -- Mobile: Use Touch simulation if needed (you may customize)
                                        -- Example: Activate buttons in GUI if available
                                    else
                                        -- PC: Send keyboard input using VirtualInputManager or similar
                                        -- For demonstration, skipped due to environment limits
                                    end

                                    -- Auto Aim + M1 (mouse button 1) simulated
                                    if AimbotOn then
                                        -- Camera aiming done in RenderStepped already
                                        -- Here you can add more attack logic if needed
                                    end

                                    -- If AutoShoot enabled, simulate mouse click or remote fire
                                    if AutoShoot and shootRemote then
                                        pcall(shootRemote.FireServer, shootRemote)
                                    end
                                end

                                -- Wait shortly before moving to next player
                                task.wait(0.3)
                            end
                        end
                    end
...
TSBTab:CreateToggle({Name="AutoShoot", CurrentValue=false, Callback=function(v) AutoShoot = v end})
...
Rayfield:Notify({Title="Nebula Hub", Content="Loaded all tabs!", Duration=3})
