-- Nebula Hub Universal - Full Version (No BABFT)
...
-- Variables for WalkSpeed & JumpPower fix
...
-- Update humanoid stats on respawn to fix freezing
...
Utility:CreateSlider({ Name = "Walk Speed", Range = {16, 200}, CurrentValue = WalkSpeedValue, Callback = function(v)
    WalkSpeedValue = v
...
Utility:CreateSlider({ Name = "Jump Power", Range = {50, 300}, CurrentValue = JumpPowerValue, Callback = function(v)
    JumpPowerValue = v
    local humanoid = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
    if humanoid then
        humanoid.UseJumpPower = true
...
VisualTab:CreateSlider({ Name = "FOV", Range = { 50, 300 }, CurrentValue = AimFOV, Callback = function(v) AimFOV = v end })
...
do
        ...
        while autofarmEnabled do
            ...
            if hrp then
                ...
                if TSBTargetPlayer and TSBTargetPlayer.Character then
                    ...
                    if tHRP then
                        local tween = TweenService:Create(hrp, TweenInfoTSB, { CFrame = tHRP.CFrame * CFrame.new(0, 3, 3) })
                        tween:Play()
    ...
    TSBTab:CreateToggle({ Name = "SafeFly", CurrentValue = false, Callback = function(v)
        if v then
            spawn(enableSafeFly)
        else
            SafeFly = false
...
-- [BloxFruits] (Fully intact)
BloxFruits:CreateButton({ Name = "Auto Farm (Placeholder)", Callback = function()
    -- Placeholder, no BABFT features here
    Rayfield:Notify({ Title = "BloxFruits", Content = "No BABFT features included", Duration = 3 })
end })

-- Notify on load
Rayfield:Notify({ Title = "Nebula Hub Universal", Content = "Fully loaded with all features except BABFT", Duration = 4 })
