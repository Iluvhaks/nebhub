...
-- AntiGrab toggle: drops any grabs immediately
...
-- Spawn Kill All toggle: grabs all players, teleports to water kill zone, releases
FTAPTab:CreateToggle({
    Name = "Spawn Kill All",
    CurrentValue = false,
        ...
        if spawnKillAll then
            spawn(function()
                ...
                while spawnKillAll do
                    for _, grab in pairs(workspace:GetChildren()) do
                        if grab.Name == "GrabParts" and grab:FindFirstChild("GrabPart") then
                            local part1 = grab.GrabPart
                            -- Release if grabbed
                            if part1:FindFirstChild("WeldConstraint") then
                                part1.WeldConstraint:Destroy()
                            end

                            -- Find closest player to grab
                            local closestPlayer = nil
                            local closestDist = math.huge
                            for _, player in pairs(Players:GetPlayers()) do
                                if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                                    local dist = (player.Character.HumanoidRootPart.Position - part1.Position).Magnitude
                            ...
                            if closestPlayer then
                                -- Move grab part to player's HRP
                                part1.CFrame = closestPlayer.Character.HumanoidRootPart.CFrame
                                task.wait(0.1)

                                -- Mobile-friendly grab
                                pcall(function()
                                    firetouchinterest(closestPlayer.Character.HumanoidRootPart, part1, 0)
                                    firetouchinterest(closestPlayer.Character.HumanoidRootPart, part1, 1)
                                end)

                                task.wait(0.3)

                                -- Teleport player into water
                                closestPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(waterPos)
...
-- Fling All toggle: spins all players around you for fling effect
FTAPTab:CreateToggle({
    Name = "Fling All",
    CurrentValue = false,
    Callback = function(value)
                ...
                while flingAll do
                    for _, player in pairs(Players:GetPlayers()) do
                        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                            ...
                            -- Add spinning angular velocity
                            local spin = Instance.new("BodyAngularVelocity")
                            spin.MaxTorque = Vector3.new(1e9, 1e9, 1e9)
                            spin.AngularVelocity = Vector3.new(0, flingAllSpinSpeed, 0)
                            spin.Parent = hrp

                            -- Position near local player for collisions
                            hrp.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(0, 0, 2)
...
-- Detect when someone grabs you, instantly drop if AntiGrab enabled
