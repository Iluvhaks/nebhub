-- Nebula Hub Universal Full Script (All Tabs + FTAP Auto Delete Fling + TSB Autofarm)
...
-- ===== Remote =====
RemoteTab:CreateButton({ Name = "Toggle Remote Lag", Callback = function() remLag=not remLag; Rayfield:Notify({Title="Remote Lag",Content=remLag and "Enabled" or "Disabled",Duration=2}); if remLag then spawn(function() while remLag do for _,o in ipairs(workspace:GetDescendants()) do if o:IsA("RemoteEvent") or o:IsA("RemoteFunction") then pcall(function() if o:IsA("RemoteEvent") then o:FireServer("NebulaSpam") else o:InvokeServer("NebulaSpam") end end) end end task.wait(0.05) end end) end end })
RemoteTab:CreateButton({ Name = "Scan Remotes", Callback = function() for _,o in ipairs(workspace:GetDescendants()) do if o:IsA("RemoteEvent") or o:IsA("RemoteFunction") then print("[Remote] "..o:GetFullName()) end end end })
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        if p~=LocalPlayer and p.Character and p.Character:FindFirstChild(TargetPart) then
            ...
            if ESPOn and on and vis then
                ...
                d.box.Visible=true; d.box.Color=Color3.new(1,0,0); d.box.Thickness=2; d.box.Size=Vector2.new(size,size); d.box.Position=Vector2.new(pos.X,pos.Y)-d.box.Size/2
            ...
            if AimbotOn and vis then
                ...
                if AutoShoot then
                    if shootRemote then pcall(shootRemote.FireServer,shootRemote) else sendVirtualInput(Enum.UserInputType.MouseButton1) end
                end
            end
        end
    end
...
-- ===== Exploits =====
Exploits:CreateButton({ Name="Click Delete", Callback=function() local m=LocalPlayer:GetMouse() m.Button1Down:Connect(function() if m.Target and m.Target.Parent:FindFirstChild("Humanoid") then m.Target.Parent:BreakJoints() end end) end })
...
FTAPTab:CreateToggle({ Name="Auto Delete Fling", CurrentValue=false, Callback=function(v)
    autoFTAPDelete=v
end })
    ...
    while autofarmEnabled do
        ...
        if hrp then
            if not TSBTargetPlayer or not TSBTargetPlayer.Character or TSBTargetPlayer.Character:FindFirstChildOfClass("Humanoid").Health<=0 then
                ...
                for _,plr in ipairs(Players:GetPlayers()) do if plr~=LocalPlayer and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                    local h=plr.Character:FindFirstChildOfClass("Humanoid") if h and h.Health>0 then local d=(hrp.Position-plr.Character.HumanoidRootPart.Position).Magnitude if d<dist then dist,nearest=d,plr end end end end
...
Rayfield:Notify({Title="Nebula Hub Universal",Content="Loaded all tabs with FTAP auto-delete fling!",Duration=3})
