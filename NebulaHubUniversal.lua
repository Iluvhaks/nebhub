...
local ESPOn, LineESP, AimbotOn, AutoShoot = false, false, false, false
...
-- Find shoot remote (for AutoShoot)
...
-- Find attack remotes (for autofarm attacks)
                    ...
                    if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                            ...
                            spinConnection = RunService.Heartbeat:Connect(function(dt)
                                ...
                                root.CFrame = hrp.CFrame * CFrame.Angles(0, math.rad(rot), 0) + Vector3.new(0, flingStrength, 0)
                            end)
                        end
                        task.wait(0.5)
                    end
...
-- TSB Autofarm (Full loop, no team checks)
TSBTab:CreateToggle({
    Name = "TSB Autofarm (Attack Anyone)",
    CurrentValue = false,
    Callback = function(v)
        autofarmEnabled = v
        if v then
            spawn(function()
                while autofarmEnabled do
                        ...
                        if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild(TargetPart) then
                            ...
                            if hum and hum.Health > 0 then
                                ...
                                while hum.Health > 0 and autofarmEnabled do
                                    ...
                                    if not success then
                                        warn("Tween error: "..tostring(err))
                                        break
                                    end
                                    task.wait(0.1)
                                end
                                print("Target killed or autofarm off, moving to next")
                                break
                            end
                        end
                    end
                    ...
                    if not targetFound then
                        print("No valid target found, waiting 2 seconds")
                        task.wait(2)
                    end
                end
                print("Autofarm disabled, exiting loop")
            end)
        end
    end
})
