-- Nebula Hub Universal Full Script (All Tabs + Fixed Fling on Grab Release + Fixed TSB Autofarm)
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        if p~=LocalPlayer and p.Character and p.Character:FindFirstChild(TargetPart) then
            ...
            if AimbotOn and vis then
                ...
                if AutoShoot then
                    if shootRemote then pcall(shootRemote.FireServer,shootRemote)
                    else sendVirtualInput(Enum.UserInputType.MouseButton1) end
                end
            end
        end
    end
...
Exploits:CreateToggle({ Name="No Clip", CurrentValue=false, Callback=function(v)
    local conn
    if v then conn=RunService.Stepped:Connect(function()
        for _,part in ipairs(LocalPlayer.Character:GetChildren()) do
            if part:IsA("BasePart") then part.CanCollide=false end
        end
    end) else if conn then conn:Disconnect() end end
end})
...
workspace.ChildAdded:Connect(function(obj)
    if obj.Name == "GrabParts" and obj:FindFirstChild("GrabPart") then
        local grab = obj.GrabPart
        local weld = grab:FindFirstChildWhichIsA("WeldConstraint") or grab:FindFirstChildWhichIsA("Weld")
...
local LowHP, RecoverHP = 0.35, 0.55
local SafeFly, SafeFlyHeight, SafeFlySpeed = false, 1000, 50
...
-- ===== Blox Fruits Tab =====
BloxFruits:CreateLabel("AutoFarm coming soon!")

Rayfield:Notify({ Title="Nebula Hub Universal", Content="All tabs loaded with fixes!", Duration=3 })
