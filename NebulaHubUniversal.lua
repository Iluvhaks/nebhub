-- Nebula Hub Universal - Full + BABFT Autofarm Tab
-- Made by Elden and Nate, now with BABFT added
...
do
    ...
    TSBTab:CreateToggle({ Name = "Enable Autofarm", CurrentValue = false, Callback = function(v)
        autofarmEnabled=v
        Rayfield:Notify({Title = "TSB Autofarm", Content = v and "Enabled" or "Disabled", Duration=2})
        if v then spawn(autofarmTSB) end
    end })
    TSBTab:CreateButton({ Name = "SafeFly Toggle", Callback = function()
        SafeFly = not SafeFly
        if SafeFly then spawn(enableSafeFly) end
        Rayfield:Notify({Title = "SafeFly", Content = SafeFly and "Enabled" or "Disabled", Duration=2})
    end })
end

-- Blox Fruits placeholder
BloxFruits:CreateLabel("Coming Soon...")

-- BABFT Autofarm Tab (Flying Block)
...
BABFT:CreateSlider({
    ...
    Range = {100, 1000},
    Increment = 10,
    CurrentValue = AF_Height,
    Callback = function(v) AF_Height = v end
})
BABFT:CreateSlider({
    Name = "Farm Speed",
    Range = {10, 100},
    Increment = 5,
    CurrentValue = AF_Speed,
    Callback = function(v) AF_Speed = v end
})
BABFT:CreateToggle({
    Name = "Start Flying Autofarm",
    CurrentValue = false,
    Callback = function(v)
        BABFTFarmRunning = v
        if v then
            spawn(function()
                ...
                while BABFTFarmRunning do
                    local stages = workspace:FindFirstChild("Boats"):FindFirstChild("DefaultBoat") and workspace.Boats.DefaultBoat:GetChildren()
                    ...
                    if BABFTFarmRunning and endZone then
                        local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
...
-- Final UI notify
Rayfield:Notify({ Title = "Nebula Hub Universal", Content = "Loaded with BABFT", Duration = 3 })
